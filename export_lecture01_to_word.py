# -*- coding: utf-8 -*-
"""
export_lecture01_to_word.py — 第1回講義読み原稿(v2)をWord形式に変換
"""
import sys, os, re
...
MD_PATH = os.path.join(SCRIPT_DIR, "lecture_script_01_v2.md")
OUT_PATH = os.path.join(SCRIPT_DIR, "セミナー読み原稿_第1回_v3.docx")
...
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run("はじめの一歩 — AIと友達になる日")
run.font.size = Pt(18)
run.font.bold = True
...
for line in lines:
    ...
    # H1
    if line.startswith('# 第') or line.startswith('# 補足') or line.startswith('# 原稿') or line.startswith('# 概要') or line.startswith('# オープニング') or line.startswith('# セクション') or line.startswith('# クロージング'):
